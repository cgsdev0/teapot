...
impl Renderer {
    ...
    fn update_display(
        ...
        io.display_size = [
            raylib_handle.get_screen_width() as _,
            raylib_handle.get_screen_height() as _,
        ];
                    ...
                    if io.mouse_draw_cursor || imgui_cursor.is_none() {
                            ...
                            .contains(ConfigFlags::NO_MOUSE_CURSOR_CHANGE)
                        {
                            if let Some(cursor) = imgui_cursor {
                                ...
                                raylib_handle
                                    .set_mouse_cursor(consts::MouseCursor::MOUSE_CURSOR_DEFAULT);
                            }
                        }
                    }
                }
    ...
    fn process_events(
        ...
        let is_control_pressed = raylib_handle.is_key_down(KeyboardKey::KEY_RIGHT_CONTROL)
            | raylib_handle.is_key_down(KeyboardKey::KEY_LEFT_CONTROL);
        ...
        let is_shift_pressed = raylib_handle.is_key_down(KeyboardKey::KEY_RIGHT_SHIFT)
            | raylib_handle.is_key_down(KeyboardKey::KEY_LEFT_SHIFT);
        ...
        let is_alt_pressed = raylib_handle.is_key_down(KeyboardKey::KEY_RIGHT_ALT)
            | raylib_handle.is_key_down(KeyboardKey::KEY_LEFT_ALT);
        ...
        let is_super_pressed = raylib_handle.is_key_down(KeyboardKey::KEY_RIGHT_SUPER)
            | raylib_handle.is_key_down(KeyboardKey::KEY_LEFT_SUPER);
        ...
        if !io.want_set_mouse_pos {
            io.add_mouse_pos_event([
                raylib_handle.get_mouse_x() as _,
                raylib_handle.get_mouse_y() as _,
            ]);
        }
        ...
        if io.config_flags.contains(ConfigFlags::NAV_ENABLE_GAMEPAD)
            && raylib_handle.is_gamepad_available(0)
        {
    ...
    unsafe fn render_triangles(
        count: usize,
        indx_start: usize,
        vtx_start: usize,
        indx_buffer: &[DrawIdx],
        vert_buffer: &[DrawVert],
    ...
    fn reload_fonts_impl(
        ...
        unsafe {
            ...
            let font_texture = raylib_handle
