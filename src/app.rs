    ...
    pub fn lightmap(&mut self, light: &Point, model: &Model, render: &Render) {
        ...
        for face in model.faces.iter() {
            ...
            for f in faces.iter() {
                ...
                if a && b && c {
                    self.shadowed.insert(face.id, 1.0);
                }
            }
        }
    }
...
impl AppState {
        ...
        r.with_raylib(&mut |_| {
            if let Some(channel) = &self.channel {
                for msg in channel.try_iter() {
                    match msg {
                        ProgressMessage::Model(model) => {
                            println!("got model msg");
                            self.model = Some(model);
                        }
                        ProgressMessage::Render(render) => {
                            println!("got render msg");
                            self.render = Some(render);
                        }
                        ProgressMessage::Lighting(_) => {
        ...
        match (&self.model, &self.render, &self.lighting) {
            ...
            (Some(model), Some(render), Some(lighting)) => {
                for contour in lighting.longitude_contours.iter() {
                    let face = &model.faces[contour.face_id];
    ...
    pub fn restart(&mut self) {
        ...
        std::thread::spawn(move || {
            // sender.send(ProgressMessage::Render(Render::default()));
            // sender.send(ProgressMessage::Lighting(Lighting::default()));
            // state.edges.clear();
            // state.faces.clear();
            // state.model_bb = BoundingBox::new();
            // state.screen_bb = BoundingBox::new();
            // state.latitude_contours.clear();
            // state.longitude_contours.clear();
...

            {
                ...
                sender.send(ProgressMessage::Model(model)).unwrap();
            }
            ...
            render.partial_culling(&model);
            {
                ...
                sender.send(ProgressMessage::Render(render)).unwrap();
            }

            let mut lighting = Lighting::default();
            // lighting.find_latitude_contours();
            // lighting.find_longitude_contours();
            // lighting.lightmap();
            {
                // progress report!
                // sender.send(ProgressMessage::Lighting(lighting)).unwrap();
            }
        });
    }
}
